...
public class AccountAspect {
	...
	public void validateWithdrawal(ProceedingJoinPoint proceedingJoinPoint) {
		...
		if (amount < minimumWithdrawal) {
			...
			System.out
					.println("Withdrawal Denied: Minimum withdrawal amount is "
							+ minimumWithdrawal);
			...
			System.out
					.println("Withdrawal Denied: Withdrawal amount is more than current balance");
		} else if (newBalance < minimumBalance) {
			try {
				logDenial(account.getAccountNumber(), amount, new Date());
				System.out
						.println("Withdrawal Denied: Balance will drop below minimum");
			} catch (IOException e) {
			}
		} else {
			try {
	...
	@After("execution(* Account.withdraw(double)) || execution(* Account.deposit(double))")
	private void printBalance(JoinPoint joinPoint) {
		Account account = (Account) joinPoint.getTarget();
		System.out.println(String.format("Account %1$s: %2$s",
				account.getAccountNumber(), account.getBalance()));
	}

	private void logDenial(String accountNumber, double amount, Date date)
			throws IOException {
